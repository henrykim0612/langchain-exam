...
def reflect(state: State) -> State:
    ...
    translated = [reflection_prompt, state['messages'][0]] + [cls_map[msg.__class__](content=msg.content) for msg in
                                                              state['messages'][1:]]
    answer = model.invoke(translated)
    # 비평을 사람이 썼다고 생각하게 우회한다.
...
initial_state = {
    'messages': [
        HumanMessage(
            content='오늘날 \'원자력 발전소\'가 왜 중요한지에 대해 에세이를 작성하세요.'
        )
    ]
}
